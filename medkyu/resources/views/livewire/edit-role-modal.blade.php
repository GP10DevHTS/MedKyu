<div>
    ...
    <x-dialog-modal wire:model="isModalOpen">
        ...
        <x-slot name="footer">
            <x-button 
            disabled title="still under development"
            {{-- title="give all permissions" --}}
             wire:click="grantAll('{{$role->id}}')" class="bg-green-400 hover:bg-green-500"  wire:click="closeModal">
                Grant All
            </x-button>
            <x-danger-button 
            disabled title="still under development"
            {{-- title="remove all permissions"  --}}
            wire:click="revokeAll('{{$role->id}}')" class="ml-2 mr-2"  wire:click="closeModal">
                Revock All
            </x-danger-button>
